...
describe('translationsRelativePath', () => {
  it('builds the per-locale file path, including ja / ja-latn', () => {
    expect(translationsRelativePath('one-piece', 'en', 'character', 'monkey-d-luffy'))
      .toBe('data/universes/one-piece/translations/en/character/monkey-d-luffy.json');
    expect(translationsRelativePath('one-piece', 'ja-latn', 'character', 'monkey-d-luffy'))
      .toBe('data/universes/one-piece/translations/ja-latn/character/monkey-d-luffy.json');
  });
});
...
describe('readTranslationRecord', () => {
  it('reads all four locale files; missing locales become {}', async () => {
    const files: Record<string, string> = {
      'data/universes/one-piece/translations/en/character/monkey-d-luffy.json': JSON.stringify({
        'character.monkey-d-luffy.name.common': 'Monkey D. Luffy',
      }),
      'data/universes/one-piece/translations/ja/character/monkey-d-luffy.json': JSON.stringify({
        'character.monkey-d-luffy.name.common': 'モンキー・D・ルフィ',
      }),
      'data/universes/one-piece/translations/ja-latn/character/monkey-d-luffy.json': JSON.stringify(
        {
          'character.monkey-d-luffy.name.common': 'Monkī Dī Rufi',
        },
      ),
    };
    ...
    expect(record.en['character.monkey-d-luffy.name.common']).toBe('Monkey D. Luffy');
    expect(record.fr).toEqual({});
    expect(record.ja['character.monkey-d-luffy.name.common']).toBe('モンキー・D・ルフィ');
    expect(record['ja-latn']['character.monkey-d-luffy.name.common']).toBe('Monkī Dī Rufi');
  });
...
describe('translationExtraFiles (save side)', () => {
  it('round-trips ja / ja-latn: what the record holds is what gets written', async () => {
    const written = translationExtraFiles('one-piece', 'character', 'monkey-d-luffy', {
      en: { 'character.monkey-d-luffy.name.common': 'Monkey D. Luffy' },
      ja: { 'character.monkey-d-luffy.name.common': 'モンキー・D・ルフィ' },
      'ja-latn': { 'character.monkey-d-luffy.name.common': 'Monkī Dī Rufi' },
    });
    expect(written.map((f) => f.path)).toEqual([
      'data/universes/one-piece/translations/en/character/monkey-d-luffy.json',
      'data/universes/one-piece/translations/ja/character/monkey-d-luffy.json',
      'data/universes/one-piece/translations/ja-latn/character/monkey-d-luffy.json',
    ]);
  ...
  it('skips empty strings and locales with nothing left — no empty files', () => {
    const written = translationExtraFiles('one-piece', 'character', 'monkey-d-luffy', {
      en: { 'character.monkey-d-luffy.name.common': 'Monkey D. Luffy' },
      fr: { 'character.monkey-d-luffy.name.common': '' },
      ja: {},
    });
    expect(written.map((f) => f.path)).toEqual([
      'data/universes/one-piece/translations/en/character/monkey-d-luffy.json',
  ...
  it('handles an absent translations payload', () => {
    expect(translationExtraFiles('one-piece', 'character', 'monkey-d-luffy', undefined)).toEqual(
      [],
    );
  });

  it('en/fr semantics are unchanged: files end with a trailing newline, 2-space indent', () => {
    const [file] = translationExtraFiles('one-piece', 'character', 'monkey-d-luffy', {
      en: { 'character.monkey-d-luffy.name.common': 'Monkey D. Luffy' },
    });
    expect(file?.content).toBe(
      `${JSON.stringify({ 'character.monkey-d-luffy.name.common': 'Monkey D. Luffy' }, null, 2)}\n`,
    );
  });
});
