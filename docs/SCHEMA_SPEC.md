...
| `short_labels`            | object   | no       | Locale → SHORT label, for inline use where the context already carries the distinction (« Chapter 1044 », not « Manga chapter 1044 »). Omitted → callers fall back to `labels` (ADR-127)                                     |
| `public_listing`          | boolean  | no       | Does this type belong in the PUBLIC wiki's home, listings and search? Default `true`. `false` (e.g. `streaming-platform`) hides the rubric only — the entity's page is still served and relations still link to it (ADR-127) |
