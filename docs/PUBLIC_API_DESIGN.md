...
### Field-lifecycle registry (ADR-080)

A generated, machine-readable **`field-lifecycle.json`**: for every
property, relation and vocabulary value, `introduced_in` (API version +
date), `deprecated_in?`, `removed_in?`, `renamed_from?`. Generated by
diffing the committed compat snapshots (ADR-042) per release — never
hand-maintained. Exposed at `GET /api/v1/meta/fields` and rendered as
the doc site's field changelog, so a consumer can ask "since when does
`slave_price` exist, and when was `volume` (string) removed?" without
reading git history.

### Official SDK (ADR-080)

`@onepiece-wiki/api-client` on npm, generated from each release's
archived OpenAPI (typed responses, `progression`/`lang` params,
pagination helpers), version-aligned with the API (SDK 1.4.x ↔ API
v1.4.x). Hand-written sugar stays thin; the generated core cannot
drift from the wire contract.

### Per-entity data history (ADR-080)

`GET /api/v1/:type/:slug/history` returns two layers: the
**in-universe** value history (the four axes already on every
historisable value — since / source / epistemic) and the **editorial**
history (when each change landed, contributor, PR number) emitted at
build time from the entity file's git log — no runtime GitHub calls.
...
Answered by ADR-080 (2026-06-14): field-lifecycle registry, official npm
SDK, per-entity history endpoint, and the header model (URL MAJOR +
`X-API-Version`; no per-request date pinning) are ratified as above.
